...
# Add company_age if not already calculated
if 'company_age' not in df.columns:
    df['company_age'] = df['age_last_funding_year']  # or compute it differently if needed

# ✅ Use ONLY the selected 3 columns
X = df[['funding_total_usd', 'category_code', 'company_age']]
y = df['status_encoded']
...
joblib.dump(X.columns.tolist(), 'feature_names.pkl')  # ✅ Save expected feature order

# Plot feature importance
plot_importance(best_model, max_num_features=3)
plt.tight_layout()
